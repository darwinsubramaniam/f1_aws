...
const checkEnvVars = () => {
  if (!API_GATEWAY_ENDPOINT) throw new Error("No API Gateway endpoint configured. Ensure the REACT_APP_API_GATEWAY_ENDPOINT environment variable is set correctly")
  if(!STORAGE_BUCKET) throw new Error("No storage bucket configured. Ensure the REACT_APP_STORAGE_BUCKET environment variable is set correctly")
  if (USE_AUTH) {
    if([
      AWS_USER_POOL_ID,
      AWS_USER_POOL_WEB_CLIENT_ID
    ].some(i =>  !i)) throw new Error("When using Cognito auth, ensure the REACT_APP_AWS_USER_POOL_ID and REACT_APP_AWS_USER_POOL_WEB_CLIENT_ID environment variables are set correctly")
  }
}
