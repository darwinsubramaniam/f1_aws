...
function PitRadio() {
  ...
  useDeepCompareEffect(() => {
    const loaded = radio.map(async i => {
      try {
        const fileAccessURL = await Storage.get(
          i.audio,
          {expires: radioMessageTtl*2}
        );
        return {
          ...i,
          url: fileAccessURL
    ...
    Promise.all(loaded).then(results => setData(results.filter(i => !!i)))
  }, [radio]);
            ...
            return (
              <Stack padding={2} sx={{borderBottom: "1px solid #3c4047"}} key={idx}>
                ...
                <Box sx={{background: "#383e47"}}>
                  <Waveform audio={i.url} color={`#${driver.teamColour}`} />
                </Box>
